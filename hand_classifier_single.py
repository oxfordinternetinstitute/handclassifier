...
#GUI
def OnButtonClickSingle(result):
    global total
    global articles
    global i
    global j
    ...
    text.delete(1.0, END)
    text.insert(INSERT, articles[i]["Text"])
...
def init_cat_single(master, optlist):
    global articles
    global i
    global j
    ...
    #Text boxes displaying articles to classify
    # XXX This should ideally be an HTML rendering if possible...
    # the tkhtml3 library should allow this (at the cost of changing the "Text"
    # call to something else...
    text_title = Label(master,text="", anchor="w",fg="black", justify="left", font=("Helvetica", 16))
    text_title.grid(column=0,row=0, sticky='EW', padx=10)

    text = Text(master, wrap=WORD)
    text.grid(column=0,row=1,rowspan=20,sticky='EW', padx=10)
    ...
    button = []
    for it, label in enumerate(optlist):
        button.append(Button(master, text=label, command= lambda j=label: OnButtonClickSingle(j)))
        button[-1].grid(column=1, row=23+it, sticky="W", padx=10)
    return text
...
categories = ("Crappy Category 1",
              "Mediocre Category 2",
              "Competent Category 3",
              "Magnificent Category 4")
...
text = init_cat_single(master, categories)
mainloop()
output.close()
